...
$ventasFiltradas = $pedido->getPedido()->generarReporteVentasEmpleado($fFolio, $fDesde, $fHasta, $fCliente);
...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->SetXY(30, 32);
        $this->Cell(150, 8, mb_convert_encoding("FILTROS ACTIVOS: " . strtoupper($this->filtrosText), 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
    ...
    function Footer() {
        ...
        $this->Cell(0, 10, mb_convert_encoding("Luchanos Corp ERP - Reporte de Control Interno - Generado el " . date('d/m/Y H:i'), 'ISO-8859-1', 'UTF-8'), 0, 0, 'L');
...
foreach ($ventasFiltradas as $v) {
    ...
    // TARJETA DE INFORMACIÓN (Más robusta)
    ...
    $pdf->Cell(90, 7, mb_convert_encoding("Fecha de Registro: " . date("d/m/Y", strtotime($v['fechayhora'])) . "  ", 'ISO-8859-1', 'UTF-8'), 0, 1, 'R');
    
    $pdf->SetFont('Arial', '', 9);
    $pdf->Cell(90, 6, mb_convert_encoding("  Metodo de Pago: " . strtoupper($v['tipo_pago']), 'ISO-8859-1', 'UTF-8'), 0, 0);
    $pdf->Cell(90, 6, mb_convert_encoding("Hora: " . date("H:i:s", strtotime($v['fechayhora'])) . "  ", 'ISO-8859-1', 'UTF-8'), 0, 1, 'R');
    ...
    $pdf->Cell(100, 9, mb_convert_encoding(" DESCRIPCION DEL ARTICULO", 'ISO-8859-1', 'UTF-8'), 0, 0, 'L', true);
    ...
    foreach ($detalles as $d) {
        ...
        // PUNTO 1: El subtotal es solo cantidad * precio unitario
    ...
    $pdf->SetTextColor(0, 35, 102);
    $pdf->Cell(0, 5, mb_convert_encoding("VALIDACIÓN ELECTRÓNICA", 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');
    $pdf->SetFont('Arial', '', 7);
